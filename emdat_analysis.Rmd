...
autumn_wet_disasters_monthly <- emdat_dat %>%
  mutate(in_reg = in_the_box & in_shapefile) %>%
  filter(!`Disaster Type` == "Drought") %>%
  filter(!`Disaster Type` == "Extreme temperature") %>%
  filter(!`Disaster Type` == "Wildfire") %>%
  ...
  mutate(start_month_effective = replace(start_month_effective, start_month_effective == 0, 12)) %>%
  group_by(`Dis No`) %>%
  summarize(any_in_reg = new_any(in_reg), start_month_eff = first(start_month_effective), year = first(Year), reg = first(region), enso_value = mean(enso_Value)) %>%
  filter(start_month_eff > 8) %>%
  filter(any_in_reg == TRUE) %>%
  group_by(reg, year, start_month_eff) %>%
  summarize(count = n(), enso_value = mean(enso_value))
```

```{r}
autumn_wet_disasters_monthly %>%
  ggplot(mapping = aes(x = enso_value, y = count)) +
  geom_point() +
  geom_smooth(method = "lm") + 
  ggpubr::stat_cor()
```
```{r}
autumn_wet_disasters_monthly %>%
  ggplot(mapping = aes(x = enso_value, y = count, color = reg)) +
  geom_point() +
  geom_smooth(method = "lm") + 
  ggpubr::stat_cor() +
  facet_wrap(~reg)
