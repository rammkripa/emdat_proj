...
sample_df <- sample_data[['disaster_data']] %>%
  filter(str_detect(Country, "United States")) %>% # Change dep on country
  sample_n(5) %>%
...
A note on the kinds of locations we get:

Some hand-picked location strings from various parts of the emdat dataset read here:
...
```{r}
shape_mask <-  st_read(here("shape_data","NH_mask.shp"))
map_of_us <- rnaturalearth::ne_countries(country = "United States of America",
                         returnclass = "sf")
inbox_df %>%
  located_in_shapefile(shapefile = shape_mask) %>%
  ggplot() +
  geom_sf(data = map_of_us) +
  geom_sf(data = shape_mask, color = 'green', fill = 'yellow') +
