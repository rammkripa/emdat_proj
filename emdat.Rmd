...
files <- list.files(path = here("data"),
                    pattern = ".xlsx")
files <- c("Malaysia_Singapore_Thai.xlsx","Venezuela_Colombia.xlsx")
listy <- list()
...
```{r}
library(tidytext)
tan_dirty <- emdat_clean %>%
  filter(region == "Tanzania_Kenya")
...
listy = list()
listy[["tan1"]] = tan_clean900
listy[["tan2"]] = tan_clean900toend
tan_final = bind_rows(listy)

tan_final %>%
  unnest_wider(col = location_data) %>%
  write_csv(here("clean_data","without_box_shape","Tanzania_Kenya.csv"))
