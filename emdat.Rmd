...
files <- list.files(path = here("data"),
                    pattern = ".xlsx")
files <- c("Vietnam.xlsx","Venezuela_Colombia.xlsx")
listy <- list()
...
```{r}
library(tidytext)
ven_dirty <- emdat_clean %>%
  filter(region == "Venezuela_Colombia")
...
ven_clean900toend %>%
  ...
  write_csv(here("clean_data","Venezuela_Colombia900toend.csv"))
